...
export default function InterestsAndPreferences({
  ...
  const handleSubInterestChange = (
    ...
    if (checked) {
      ...
      const updatedSubInterests = currentSubInterests.filter(
        (item) => item !== subInterest
      );
      if (updatedSubInterests.length === 0) {
        const { [areaLabel]: _, ...remainingInterests } =
          currentDetailedInterests;
        ...
        <FormField
          ...
          render={() => (
            <FormItem>
              ...
              <FormDescription>
                Select the main areas that interest you, then choose specific
                  ...
                  {conferenceInterestAreasOptions.map((item) => (
                    <div key={item.value} className="border rounded-lg p-4">
                      <FormField
                        ...
                        name="interestAreas"
                        render={({ field }) => {
                          const isAreaSelected = field.value?.includes(
                            item.value
                          );
                          const isExpanded = expandedAreas.has(item.value);
                          const detailedInterests =
                            form.watch("detailedInterests") || {};
                          const selectedSubInterests =
                            detailedInterests[item.label] || [];
                                    ...
                                    <CollapsibleContent className="pt-3 border-t border-gray-200">
                                      <div className="pl-6 space-y-2">
                                        ...
                                        <div className="grid gap-2">
                                          {item.subInterests.map(
                                            (subInterest, index) => (
                                              <label
                                                key={index}
                                                className="flex items-start space-x-2 p-2 rounded border hover:bg-muted/10 cursor-pointer"
                                              >
                                                <Checkbox
                                                  checked={selectedSubInterests.includes(
                                                    subInterest
                                                  )}
                                                  onCheckedChange={(checked) =>
                                                    handleSubInterestChange(
                                                      item.label,
                                                      subInterest,
                                                      checked as boolean
                                                    )
