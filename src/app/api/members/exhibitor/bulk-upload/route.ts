...
// Validation schema for CSV row - matching simplified ExhibitorMembers schema
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingEmails = new Set(
      (await prisma.exhibitorMembers.findMany({
        select: { email: true }
    ...
    for (let i = 0; i < validMembers.length; i += batchSize) {
      ...
      try {
        await prisma.exhibitorMembers.createMany({
          data: batch,
