...
import { generateExhibitorCodeEmail } from '@/lib/code-email';
...
export async function POST(request: NextRequest) {
  try {
    ...
    let member;
    try {
      member = await prisma.exhibitorMembers.findUnique({
        where: { id: memberId },
      });
    } catch (dbError) {
    ...
    if (member.sentCode) {
      return NextResponse.json(
        { 
          error: 'This member already has an exhibitor code assigned',
    ...
    try {
      existingSentCode = await prisma.exhibitorCodeDistribution.findFirst({
        where: {
          sentTo: member.email,
          isSent: true,
        },
    ...
    if (existingSentCode) {
      return NextResponse.json(
        { 
          error: 'An exhibitor code has already been sent to this member',
    ...
    // Find an available exhibitor code (isActive: true and not used)
    let availableCode;
    try {
      availableCode = await prisma.exhibitorCodeDistribution.findFirst({
        where: {
          isActive: true,
          userId: null,
    ...
    if (!availableCode) {
      return NextResponse.json(
        { error: 'No available exhibitor codes found. Please contact admin to generate more codes.' },
        { status: 404 }
      );
    }

    // Generate email content
    const emailContent = generateExhibitorCodeEmail({
    ...
    const msg = {
      ...
      subject: 'Your BEACON 2025 Exhibitor Code - Registration Access',
      html: emailContent,
    };
    ...
    return NextResponse.json({
      success: true,
      message: `Exhibitor code sent successfully to ${member.email}`,
      codeSent: availableCode.code,
    });

  } catch (error) {
    console.error('Error sending exhibitor code:', error);
