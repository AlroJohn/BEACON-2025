...
interface BulkGenerateExhibitorCodesDialogProps {
...
export function BulkGenerateExhibitorCodesDialog({ 
  trigger, 
  onCodesGenerated,
}: BulkGenerateExhibitorCodesDialogProps) {
  ...
  const onSubmit = async (data: BulkGenerateFormData) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || "Failed to generate exhibitor codes");
      }
      ...
      toast.success(`Successfully generated ${result.data.summary.created} exhibitor codes!`);
      onCodesGenerated();

    } catch (error) {
      console.error("Error generating bulk codes:", error);
      toast.error(error instanceof Error ? error.message : "Failed to generate exhibitor codes");
      setProgress(0);
    } finally {
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Bulk Generate Exhibitor Codes
          </DialogTitle>
          <DialogDescription>
            Generate multiple random exhibitor codes at once. Specify the quantity and status.
          </DialogDescription>
        </DialogHeader>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Generate between 1 and 1000 exhibitor codes (recommended: 50-200 for optimal performance)
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
            ...
            <div className="text-center space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Your exhibitor codes have been created and are ready to use.
                </p>
              </div>
            </div>
